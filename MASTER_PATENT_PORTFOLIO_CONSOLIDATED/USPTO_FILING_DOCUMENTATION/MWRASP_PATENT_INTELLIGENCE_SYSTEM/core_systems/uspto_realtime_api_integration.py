...
class USPTORealtimeAPI:
    ...
    async def search_patents_advanced(self, query: PatentSearchQuery) -> List[USPTOPatentRecord]:
        ...
        print(f"[SEARCH] Executing advanced USPTO patent search...")
        ...
        try:
            ...
            if 'patents' in response_data:
                ...
                print(f"[OK] Retrieved {len(patents)} patents from USPTO API")
                ...
                print("[ERROR] No patents found or API error")
                return []
    ...
    async def _convert_to_structured_record(self, patent_data: Dict) -> Optional[USPTOPatentRecord]:
        ...
        try:
            ...
            print(f"[WARNING] Error converting patent record: {e}")
            return None
    ...
    async def get_patent_details(self, patent_number: str) -> Optional[USPTOPatentRecord]:
        ...
        print(f"[LIST] Retrieving detailed information for patent: {patent_number}")
        ...
        try:
            ...
            if 'patents' in response_data and response_data['patents']:
                ...
                if record:
                    print(f"[OK] Retrieved detailed information for {patent_number}")
                    return record
            
            print(f"[ERROR] Patent {patent_number} not found")
            return None
            
        except Exception as e:
            print(f"[ERROR] Error retrieving patent details: {e}")
            return None
    
    async def search_by_inventor(self, inventor_name: str, limit: int = 50) -> List[USPTOPatentRecord]:
        """Search patents by inventor name"""
        
        print(f"👨‍[SCIENCE] Searching patents by inventor: {inventor_name}")
...
async def integrate_uspto_realtime_search(patent_keywords: List[str], base_directory: str) -> Dict:
    ...
    print(f"[TARGET] Keywords: {', '.join(patent_keywords[:5])}")
    ...
    try:
        async with USPTORealtimeAPI(base_directory) as uspto_api:
            ...
            print(f"[OK] USPTO API integration complete: {len(patent_records)} patents")
            
    except Exception as e:
        print(f"[ERROR] USPTO API integration failed: {e}")
...
async def test_uspto_api():
    ...
    async with USPTORealtimeAPI(base_dir) as uspto_api:
        ...
        print(f"[OK] Found {len(class_results)} patents in computing/networking")
